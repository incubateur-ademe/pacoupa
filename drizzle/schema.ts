import { sqliteTable, AnySQLiteColumn, text, uniqueIndex, integer, foreignKey } from "drizzle-orm/sqlite-core"
...
export const caracteristiques = sqliteTable("caracteristiques", {
...
(table) => {
	return {
		idxCaracteristiquesChEcsEmetteurEspaceExterieurEnvContraintToitureTerrasseTemperatureNbLgtsNiveauRenovation: uniqueIndex("idx_caracteristiques_CH_ECS_emetteur_espace_exterieur_env_contraint_toiture_terrasse_temperature_nb_lgts_niveau_renovation").on(table.ch, table.ecs, table.emetteur, table.espaceExterieur, table.envContraint, table.toitureTerrasse, table.temperature, table.nbLgts, table.niveauRenovation),
	}
});
