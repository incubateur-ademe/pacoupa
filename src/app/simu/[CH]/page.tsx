...
const sql = (
  ...
  from simu1 where CH = '${CH}' and ECS = 'IND' and emetteur = 'Hydraulique' and temperature = 'Superieur à 60°C' and (niveau_renovation = 'Recent ou renove' or niveau_renovation = 'NA') and (nb_lgts = '> 15' or nb_lgts = 'NA') order by ordre_solution`;

const Page = async ({ params }: { params: { CH: string } }) => {
  // const { rows } = await turso.execute("SELECT * FROM simu1");
  const { rows } = await turso.execute(sql(params.CH));
